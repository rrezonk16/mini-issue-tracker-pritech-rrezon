...
<head>
    ...
    <title>Create Issue - PRITECH Issue Tracker</title>
...
<body class="bg-gray-50 font-sans antialiased min-h-screen flex flex-col">
    <div class="container mx-auto px-6 py-12 flex-grow">
        ...
        @endif
        <h1 class="text-2xl font-bold text-gray-800 mb-8">Create Issue for {{ $project->name }}</h1>
        <form action="{{ route('issues.store', $project) }}" method="POST" class="space-y-5 max-w-md">
            @csrf
            <div>
            ...
            <div>
                ...
                <textarea name="description" id="description"
                          class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">Create Issue</button>
            </div>
        </form>
    </div>
</body>
